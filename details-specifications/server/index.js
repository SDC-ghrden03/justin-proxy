...
app.get ( '/api/convenience-specs', function (req, res) {
    ...
    ConvenienceFeature.find( {vehicle: '5e3c6bf2304d31777840b148'}, function (err, docs) {
        console.log('This is convenience feature docs ', docs)
        res.json(docs);
    });
});

app.get ( '/api/entretainment-specs', function (req, res) {
    EntmtFeat.find( {vehicle: '5e3c6bf2304d31777840b148'}, function (err, docs) {
        res.json(docs);
    })
});

app.get ( '/api/off-road-specs', function (req, res) {
    OffRoadCapability.find( {vehicle: '5e3c6bf2304d31777840b148'}, function (err, docs) {
        res.json(docs);
    })
});

app.get( '/api/seat-trim-specs', function (req, res) {
    SeatTrim.find( {vehicle: '5e3c6bf2304d31777840b148'}, function (err, docs) {
        res.json(docs);
    })
});

app.get ( '/api/specs-dimensions', function (req, res) {
    SpecsDimen.find( {vehicle: '5e3c6bf2304d31777840b148'}, function (err, docs) {
        res.json(docs);
    })
})
